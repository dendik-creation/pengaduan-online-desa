...
class DashboardController extends Controller
{
    public function adminDashboard()
    {
        ...
        // Rating rata-rata dari ulasan berdasarkan kategori
        $ratingKepuasan = Ulasan::where("tipe", "kepuasan")->avg("nilai") ?? 0;
        $ratingKualitas = Ulasan::where("tipe", "kualitas")->avg("nilai") ?? 0;
        $ratingKecepatan =
            Ulasan::where("tipe", "kecepatan")->avg("nilai") ?? 0;
        ...
        $pengaduanBulanIni = Pengaduan::whereMonth(
            "tanggal_pengaduan",
            Carbon::now()->month,
        )
        ...
        $pengaduanSelesaiBulanIni = Pengaduan::whereMonth(
            "tanggal_pengaduan",
            Carbon::now()->month,
        )
            ->whereYear("tanggal_pengaduan", Carbon::now()->year)
        ...
        $persentasePenyelesaian =
            $pengaduanBulanIni > 0
                ? round(($pengaduanSelesaiBulanIni / $pengaduanBulanIni) * 100)
                : 0;

        // Statistik tambahan untuk ringkasan performa
        $waktuRataRataPenyelesaian = $this->getSystemAverageCompletionTime();
        $totalEksekutor = User::where("role", "eksekutor")->count();
    ...
    public function pendudukDashboard()
    {
        ...
        $pengaduanBaru = Pengaduan::where("pengguna_id", $userId)
            ->where("status", "baru")
            ->count();
        $pengaduanDiproses = Pengaduan::where("pengguna_id", $userId)
            ->where("status", "diproses")
            ->count();
        $pengaduanSelesai = Pengaduan::where("pengguna_id", $userId)
            ->where("status", "selesai")
            ->count();
        ...
        // Tingkat penyelesaian user sendiri
        $tingkatPenyelesaianUser =
            $totalPengaduanSaya > 0
                ? round(($pengaduanSelesai / $totalPengaduanSaya) * 100)
                : 0;
        ...
        $ratingRataRata =
            Ulasan::whereHas("pengaduan", function ($query) use ($userId) {
                $query->where("pengguna_id", $userId);
            })->avg("nilai") ?? 0;

        // Waktu rata-rata penyelesaian pengaduan user
        $waktuRataRataPenyelesaian = $this->getUserAverageCompletionTime(
            $userId,
        );
    ...
    private function getWeeklyProgress($userId)
    {
        ...
        for ($i = 3; $i >= 0; $i--) {
            ...
            $completed = Penugasan::where("eksekutor_id", $userId)
                ->whereHas("pengaduan", function ($query) use (
                    $weekStart,
                    $weekEnd,
                ) {
                    $query
    ...
    /**
     * Calculate system-wide average completion time
     */
    private function getSystemAverageCompletionTime()
    {
        $completedComplaints = Pengaduan::where("status", "selesai")->get();
